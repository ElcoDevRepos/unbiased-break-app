...
export class Tab3Page implements OnInit, OnDestroy {
  @ViewChild('modalSeven') purchaseModal;
  expanded = false;
  replies;
  reminders;
  randomArticle;
...
import {
  ...
  PurchasesOfferings,
  LOG_LEVEL,
  PURCHASES_ERROR_CODE, // Types for TypeScript
    ...
    this.platform.ready().then(async () => {
      ...
      if (this.platform.is('ios')) {
        await Purchases.configure({
          apiKey: 'appl_iUHpFqEZlrNrhjyZdtKdBOPfGKo',
        });
      } else if (this.platform.is('android')) {
        await Purchases.configure({
          apiKey: 'goog_lUAhvzmoLBEWVyiPYSuwUFnPUDe',
        });
      }
  ...
  async purchase() {
    try {
      const purchaseResult = await Purchases.purchasePackage({
        aPackage: this.offerings.current.availablePackages[0],
      });
      if (
